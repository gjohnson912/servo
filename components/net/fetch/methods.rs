...
pub fn main_fetch(request: &mut Request,
    ...
    if should_be_blocked_as_mixed_content(request.client.as_ref(),
                                          request.initiator,
                                          request.type_,
                                          request.destination,
                                          request.mode,
...
fn should_be_blocked_as_mixed_content(client: Option<&Client>,
                                      initiator: Initiator,
                                      type_: Type,
...
fn is_potentially_trustworthy_origin(origin: UrlOrigin) -> bool {
    ...
    // Step 6.
    // Not applicable: there are no other schemes that Servo considers
    // to be authenticated.
